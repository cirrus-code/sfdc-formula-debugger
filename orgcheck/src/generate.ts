...
function dataApex(): string {
  const lines: string[] = [
    `// Generated by orgcheck generate.ts — wipes and reloads probe records.`,
    `delete [SELECT Id FROM ${OBJECT_API_NAME} LIMIT 10000];`,
    `List<${OBJECT_API_NAME}> rs = new List<${OBJECT_API_NAME}>();`,
  ];
  let i = 0;
...
writeFileSync(join(ROOT, "data.apex"), dataApex());
