...
import { CONTEXTS, FUNCTIONS, functionArity, getContext, getFunction } from "./index.ts";
...
describe("registry: function table consistency", () => {
  it("has unique, canonical uppercase names", () => {
    ...
    for (const f of FUNCTIONS) {
      expect(f.name, `${f.name} should be uppercase`).toBe(f.name.toUpperCase());
  ...
  it("places optional and variadic params only in valid positions", () => {
    for (const f of FUNCTIONS) {
      f.params.forEach((p, i) => {
        ...
        if (p.variadic) {expect(i, `${f.name}: variadic must be last`).toBe(f.params.length - 1);}
        // No required param may follow an optional one.
        if (i > 0 && f.params[i - 1]!.optional && !p.optional && !p.variadic) {
          throw new Error(`${f.name}: required param '${p.name}' follows an optional param`);
        }
      });
    }
  });
  ...
  it("references only declared context ids", () => {
    for (const f of FUNCTIONS) {
      if (f.contexts === "all") {continue;}
      for (const id of f.contexts) {
        expect(CONTEXT_IDS.has(id), `${f.name} references unknown context '${id}'`).toBe(true);
      }
    }
  });
  ...
  it("keeps sameAsArg return rules within the parameter range", () => {
    for (const f of FUNCTIONS) {
      if (f.returnType.kind === "sameAsArg") {
        ...
        expect(f.returnType.index, `${f.name}: sameAsArg index out of range`).toBeLessThan(f.params.length);
      }
    }
  });
  ...
  it("gives every function a summary and docs URL", () => {
    for (const f of FUNCTIONS) {
      ...
      expect(f.docsUrl.startsWith("https://"), `${f.name} needs an https docs URL`).toBe(true);
    }
  });
  ...
  it("computes sane arities", () => {
    ...
    expect(functionArity(getFunction("AND")!)).toEqual({ min: 1, max: Number.POSITIVE_INFINITY });
...
describe("registry: context configs", () => {
  ...
  it("labels every Tier 2 context with an unverified-availability note", () => {
    for (const c of CONTEXTS) {
      if (c.tier === 2) {expect(c.notes, `${c.id} needs a Tier 2 note`).toBeTruthy();}
    }
  });
